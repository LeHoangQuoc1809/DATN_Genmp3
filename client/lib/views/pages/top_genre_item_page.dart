...
class TopGenreItemPage extends StatefulWidget {
  const TopGenreItemPage({
    super.key,
  ...
  @override
  State<TopGenreItemPage> createState() => _TopGenreItemPageState();
}

class _TopGenreItemPageState extends State<TopGenreItemPage> {
  final ScrollController _scrollController = ScrollController();
  bool _showTopInAppBar = false;
  bool _isLoading = true;
  bool isSwipingLeft = false;
  List<Song> _songsData = [];
  ...
  Future<void> loadData() async {
    try {
      ...
      if (widget.type == 'genre') {
        songsData = await ModelviewsManager.songModelviews.getTop100SongsListenCountMaxByGenreId(genreId: widget.id);
        print('--------------------getTop100SongsListenCountMaxByGenreId: ${widget.id}--------------------');
      } else if (widget.type == 'topic') {
        songsData = await ModelviewsManager.songModelviews.getTop100SongsListenCountMaxByTopicId(topicId: widget.id);
        print('--------------------getTop100SongsListenCountMaxByTopicId: ${widget.id}--------------------');
      }
      setState(() {
                          ...
                          child: Text(
                            _showTopInAppBar
                                ? widget.name.length <= 9
                                    ? 'Top 100 ${widget.name.toUpperCase()}'
                                    : 'Top 100'
                                : '',
                            maxLines: 1,
                            overflow: TextOverflow.ellipsis,
