...
class AlbumService {
  ...
  Future<dynamic> get50AlbumsForSearch(String query) async {
    dynamic jsonResponse;
    try {
      var url = Uri.parse('${frontUrl}albums/get-top-50-albums-for-search/${query}');
  ...
  Future<dynamic> getAlbumsByArtistId(int artist_id) async {
    dynamic jsonResponse;
    try {
      var url = Uri.parse('${frontUrl}albums/get-albums-by-artist-id/${artist_id}');
      http.Response response = await http.get(url);
      if (response.statusCode == 200) {
        dynamic responseData = jsonDecode(utf8.decode(response.bodyBytes));
        jsonResponse = {
  ...
  Future<dynamic> createAlbum(
    ...
    dynamic jsonResponse;
    try {
      final uri = Uri.parse('${frontUrl}albums/create-album/'); // Thay đổi URL của bạn ở đây
  ...
  Future<dynamic> updateAlbumById(
    ...
    dynamic jsonResponse;
    try {
      final uri = Uri.parse('${frontUrl}albums/update-album-by-id/$id'); // Thay đổi URL của bạn ở đây
      final Map<String, dynamic> songData = {
        "id": id,
        "name": name,
        "description": description,
