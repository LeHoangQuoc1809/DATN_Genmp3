...
class ArtistService {
  ...
  Future<dynamic> get50ArtistsForSearch(String query) async {
    dynamic jsonResponse;
    try {
      var url = Uri.parse('${frontUrl}artists/get-top-50-artists-for-search/${query}');
  ...
  Future<dynamic> getTop5SimilarArtist(int id) async {
    dynamic jsonResponse;
    try {
      var url = Uri.parse('${frontUrl}artists/get-top-5-artists-similar-by-id/$id');
      http.Response response = await http.get(url);
      if (response.statusCode == 200) {
        dynamic responseData = jsonDecode(utf8.decode(response.bodyBytes));
        jsonResponse = {
  ...
  Future<dynamic> createArtist({
    ...
    dynamic jsonResponse;
    try {
      final uri = Uri.parse('${frontUrl}artists/create-artist/'); // Thay đổi URL của bạn ở đây
  ...
  Future<dynamic> updateArtist({
    ...
    dynamic jsonResponse;
    try {
      final uri = Uri.parse('${frontUrl}artists/update-artist-by-id/$id'); // Thay đổi URL của bạn ở đây
      final Map<String, dynamic> songData = {
        "name": name,
        "description": description,
        "picture": picture,
