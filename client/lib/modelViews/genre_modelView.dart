...
class GenreModelview {
  ...
  Future<List<Genre>> getTop3Genres() async {
    List<Genre> genres = [];
    final data = await ServiceManager.genreService.getTop3Genres();
    if (data['message'] == "OK") {
      var list = data['data'] as List;
      genres = list.map((i) => Genre.fromJson(i)).toList();
    } else {
...
// }) async {
//   final data = await ServiceManager.genreService.createGenre(name: name, description: description);
//   Genre? newGenre;
//   if (data['message'] == "OK") {
...
//   required String description,
// }) async {
//   final data = await ServiceManager.genreService.updateGenreById(id: id, name: name, description: description);
//   Genre? newGenre;
//   if (data['message'] == "OK") {
//     newGenre = Genre.fromJson(data['data']);
//   } else {
