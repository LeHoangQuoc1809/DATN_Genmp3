import 'package:app/configs/dateFormat.dart';
...
import 'playlist_feature_dialog.dart';

class PlaylistContentPanel extends StatefulWidget {
  RSS rss;

  PlaylistContentPanel({super.key, required this.rss});

  @override
  State<PlaylistContentPanel> createState() => _PlaylistContentPanelState();
}

class _PlaylistContentPanelState extends State<PlaylistContentPanel> {
  ...
  Future loadData() async {
    ...
    List<Playlist> playlist = await ModelviewsManager.playlistModelview.getAllPlaylists();
  ...
  void updateListViewContentForSearching(List<dynamic> newData, bool isaSsign) {
    setState(() {
      if (newData.isEmpty && isaSsign) {
        ...
        _playlist = newData.map((i) => i as Playlist).toList();
      } else {
        loadData();
      }
    });
  ...
  Future<bool> deleteArtist(Playlist playlist) async {
    bool state = await ModelviewsManager.artistModelview.deleteArtistById(playlist.id);
    return state;
  }

  Future<void> onTapDelete(Playlist playlist) async {
  ...
  Future<void> onTapEdit(Playlist playlist) async {
    final result = await showGeneralDialog<int>(
      context: context,
  ...
  Future<void> onTapDetail(Playlist playlist) async {
    final result = await showGeneralDialog<int>(
      context: context,
      barrierDismissible: false,
      transitionDuration: const Duration(milliseconds: 300),
  ...
  //#################################
  List<String> headers = Playlist.getFields()..add("Actions");
  ...
  Widget _buildTableContentRow({
    ...
    Future<void> Function(Playlist)? onTapDelete,
    Future<void> Function(Playlist)? onTapEdit,
    Future<void> Function(Playlist)? onTapDetail,
          ...
          Row(
            children: [
              CircelPictureComponent(
                rss: rss,
                flex: 3,
                src: Playlist.getUrlImg(playlist.picture, playlist.id),
              ),
              Expanded(
                flex: 1,
                child: Container(
              ...
              Expanded(
                ...
                child: Container(
                  padding: edgeInsets,
                  child: Text(
                    DateFormatConfig.DateFormatForDisplay(playlist.modifyDate),
                    style: styleContentRow,
                    maxLines: 1,
                    overflow: TextOverflow.ellipsis,
                  ),
              ...
              Expanded(
                ...
                child: ActionsButtonRow(
                  onTapDelete: onTapDelete != null ? (object) => onTapDelete!(object as Playlist) : null,
                  onTapEdit: onTapEdit != null ? (object) => onTapEdit!(object as Playlist) : null,
                  onTapDetail: onTapDetail != null ? (object) => onTapDetail!(object as Playlist) : null,
                  object: playlist,
